<svelte:head>
	<title>Theming &amp; Styling - ONE-IL Documentation</title>
	<meta
		name="description"
		content="Learn how ONE-IL uses Tailwind CSS 4 with OKLCH colors, native CSS theming, and dark mode — all powered by Svelte 5 and SvelteKit."
	/>
</svelte:head>
...
<p>
	ONE-IL uses <strong>Tailwind CSS 4</strong> with a native CSS theming system — there is no
	JavaScript configuration file. All colors are defined using the
...
<p>
	Tailwind CSS 4 is a ground-up rewrite that moves configuration into native CSS. ONE-IL Admin's
	setup lives entirely in <code>src/app.css</code>:
</p>
...
<p>
	On the build side, ONE-IL uses <code>@tailwindcss/vite</code> as a Vite plugin. This replaces the
	PostCSS-based setup from Tailwind v3 and provides faster compilation integrated directly into the
	<strong>SvelteKit</strong> dev server.
</p>

<h2>OKLCH Color System</h2>

<p>
	All colors in ONE-IL are defined in the <strong>OKLCH</strong> color space rather than the traditional
	HSL. OKLCH stands for:
</p>
...
<p>
	...
	utility classes. ONE-IL defines this in <code>src/app.css</code>:
</p>
...
<p>
	ONE-IL uses class-based dark mode powered by the <strong>mode-watcher</strong> library. Here is
	how all the pieces fit together in the <strong>SvelteKit</strong> application:
</p>
...
<p>
	ONE-IL uses the <strong>system font stack</strong> via Tailwind's default
...
<p>
	ONE-IL follows a <strong>mobile-first</strong> approach. Base styles target small screens, and responsive
	breakpoints add complexity for larger viewports.
</p>
...
<p>
	ONE-IL applies two global base styles via Tailwind's <code>@layer base</code>
	directive:
</p>
