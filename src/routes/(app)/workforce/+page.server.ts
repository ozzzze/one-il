...
export const load: PageServerLoad = async ({ locals }) => {
	assertPermission(locals.user, "workforce:manage");
	...
	const employees = (employeesRes.data ?? []) as unknown as WorkforceEmployee[];
...
export const actions: Actions = {
	createEmployee: async ({ request, locals }) => {
		...
		if (!parsed.success) {
			return fail(400, { message: "Employee data is invalid" });
		}
		...
		const { error } = await admin.from("employees").insert({
			...
			employee_no: payload.employeeNo || null,
		});
	...
	assignPrimary: async ({ request, locals }) => {
		...
		if (!parsed.success) {
			return fail(400, { message: "Assignment data is invalid" });
		}
	...
	setSupervisor: async ({ request, locals }) => {
		...
		if (!parsed.success) {
			return fail(400, { message: "Supervisor data is invalid" });
		}
	...
	assignProgramChair: async ({ request, locals }) => {
		...
		if (!parsed.success) {
			return fail(400, { message: "Program chair data is invalid" });
		}
