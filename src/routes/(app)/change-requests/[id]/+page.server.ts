...
import { parseChangeRequestForm, parseItImplementForm } from '$lib/server/one-leave/change-request/schemas.js';
import {
...
export const actions: Actions = {
	...
	uploadAttachment: async (event) => {
		...
		const canUploadDraft =
			canEditScr(user, record) && attachmentType === 'request_supporting';
		const canUploadTest =
			(canItImplement(user, record) ||
				canEmergencyImplement(user, record)) &&
