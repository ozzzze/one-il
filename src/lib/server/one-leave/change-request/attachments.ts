...
export async function deleteScrAttachment(
	...
	const pool = await getDbPool();
	const findResult = await pool.request().input("id", attachmentId).input("scrId", scrId).query<{
		storage_path: string;
	}>(`
			SELECT [storage_path]
			FROM [one_leave].[system_change_attachments]
...
export async function readScrAttachmentPath(
	...
	const pool = await getDbPool();
	const result = await pool.request().input("id", attachmentId).input("scrId", scrId).query<{
		storage_path: string;
		file_name: string;
		mime_type: string;
	}>(`
